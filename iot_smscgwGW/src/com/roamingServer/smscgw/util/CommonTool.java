...
public class CommonTool {
	...
	public static void bytesCopyTo(byte[] srcBytes, byte[] destBytes,
		...
		int n = srcInx + len - srcBytes.length;
		if (n < 0) {
			n = 0;
		}
		if (CMPP_FILL_ZERO == 0) {
	...
	public static byte[] longToBytes5(long i) {
		...
		abyte[4] = (byte) (0xff & i);
		abyte[3] = (byte) ((0xff00 & i) >> 8);
		abyte[2] = (byte) ((0xff0000 & i) >> 16);
		abyte[1] = (byte) ((0xff000000 & i) >> 24);
		abyte[0] = (byte) ((0xff00000000l & i) >> 32);
		return abyte;
	}
	...
	public static boolean isChinese(String tmp) {
		...
		for (int i = 0; i < tmp.getBytes().length; i++) {
			if ((tmp.getBytes()[i] > 127) || (tmp.getBytes()[i] < 0)) {
				tag = true;
			}
		}
		return tag;
